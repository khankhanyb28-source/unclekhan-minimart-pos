...
import cn.com.zj.command.sdk.WifiCommunication1;
...
public class PrintDemo extends Activity {
	...
	private WifiCommunication1 wfComm = null;
...
/******************************************************************************************************/
	...
	private void checkPermissions(String... permissions) {
		...
		if (null != needRequestPermissonList && needRequestPermissonList.size() > 0) {
			ActivityCompat.requestPermissions(PrintDemo.this, needRequestPermissonList.toArray(
	...
	private List<String> findDeniedPermissions(String[] permissions) {
		...
		for (String perm : permissions) {
			if (ContextCompat.checkSelfPermission(PrintDemo.this,perm) != PackageManager.PERMISSION_GRANTED) {
				needRequestPermissonList.add(perm);
			} else {
				if (ActivityCompat.shouldShowRequestPermissionRationale(PrintDemo.this, perm)) {
					needRequestPermissonList.add(perm);
				}
			}
		}
	...
	public void onCreate(Bundle savedInstanceState){  
	    ...
	    wfComm = new WifiCommunication1(mHandler);
	    ...
	    String result = getLanguaeLocal(this);
	    Log.e("aa",result);
	}   

	public String getLanguaeLocal(Context context){
...
//                }).start();
				...
				if(txt_msg.length() == 0){
					...
					if(width_58mm.isChecked()){
						final Bitmap bmp = Other.createAppIconText(bm1,txt_msg,24,is58mm,200);
						imageViewPicture.setImageBitmap(bmp);
						final byte[] buffer = PrinterCommand.POS_Set_PrtInit();
						final byte[] sp = PrinterCommand.POS_Set_LineSpace(0);
						final int nMode = 0;
						final int nPaperWidth = 384;
						...
						final Bitmap bmp = Other.createAppIconText(bm1,txt_msg,24,false,200);
						imageViewPicture.setImageBitmap(bmp);
						final byte[] buffer = PrinterCommand.POS_Set_PrtInit();
						final byte[] sp = PrinterCommand.POS_Set_LineSpace(0);
						final int nMode = 0;
						
						final int nPaperWidth = 576;
						if(bmp != null)
						{
...
/****************************************************************************************************/
	...
	private void Print_BMP(){
		...
		if(mBitmap != null)
		{
			final byte[] data = PrintPicture.POS_PrintBMP(mBitmap, nPaperWidth, nMode);
	...
	private void BluetoothPrintTest() {
		...
		Log.e("BluetoothPrintTest","BluetoothPrintTest");
		String msg = "";
		String lang = getString(R.string.strLang);
...
/************************************************************************************************/
	private void SendDataByte(final byte[] data){
		new Thread(new Runnable() {
			@Override
			public void run() {
				wfComm.sndByte(data);
	...
	private void SendDataString(final String data){
	    Log.e("SendDataString","SendDataString");
	    new Thread(new Runnable() {
            @Override
            public void run() {
    ...
    private void createImage() {
	        try {
	         ...
	         //   Log.i(TAG, "生成的文本：" + text);
	            ...
	            final byte[] data = PrintPicture.POS_PrintBMP(bitmap, 384, 0);
	            new Thread(new Runnable() {
					@Override
					public void run() {
						wfComm.sndByte(data);
