...
public class Main_Activity extends Activity implements OnClickListener {
    ...
    private static final String EYU = "cp866";
    ...
    private RadioButton width_58mm, width_80, width_110mm;
    private RadioButton thai, big5, Simplified, Korean, eyu;
    ...
    final String[] items = {"复位打印机", "打印并走纸", "标准ASCII字体", "压缩ASCII字体", "正常大小",
            "倍高", "倍宽", "二倍高倍宽", "三倍高倍宽", "四倍高倍宽", "取消加粗模式", "选择加粗模式", "取消倒置打印", "选择倒置打印", "取消黑白反显", "选择黑白反显",
    ...
    private void KeyListenerInit() {
        ...
        width_110mm = findViewById(R.id.width_110mm);
        width_110mm.setOnClickListener(this);
        ...
        eyu = findViewById(R.id.eyu);
        eyu.setOnClickListener(this);
                ...
                if (hexBox.isChecked()) {
                    ...
                    if (msg.length() > 0) {
                        if (thai.isChecked()) {
                            ...
                            SendDataString(msg);
                            SendDataByte(Command.LF);
                        } else if (eyu.isChecked()) {
                            SendDataByte(PrinterCommand.POS_Print_Text(msg, EYU, 17, 0, 0, 0));
                            //SendDataStringEncode("сегодня",EYU);
                            SendDataByte(Command.LF);
                        }
...
//            }
            ...
            case R.id.width_110mm: {
                //is58mm = v == width_58mm;
                width_58mm.setChecked(false);
                width_80.setChecked(false);
                width_110mm.setChecked(true);
                break;
            }
...
//                                public void run() {
//                                    if(isReceive==false){
//                                        Toast.makeText(Main_Activity.this,"打印失败!!!",Toast.LENGTH_SHORT).show();
//
//                                    }else{
//                                        Toast.makeText(Main_Activity.this,"打印成功!!!",Toast.LENGTH_SHORT).show();
//                                    }
//
//                                    isReceive = false;
//                                }
...
/****************************************************************************************************/
     ...
     * @param
     */
    private void BluetoothPrintTest1(boolean isForward) {
        if(isForward){
            SendDataByte(Command.ESC_Init);
        }else{
            SendDataByte(Command.ESC_Init1);
        }
        //SendDataByte(PrinterCommand.POS_Set_PrintWidth(100));

        SendDataString(" 送货时间: 2022-01-22\n");
        SendDataString(" 客户地址: 广东省深圳市龙华新区明治街道xxxxx号门牌xxxxxx\n");
        SendDataString(" 客户电话: 12345678901\n");

        SendDataString("------------------------------------------------------------------\n");

        SendDataString(" 名称          单价          数量          总价\n");
        SendDataString(" 茴香          8元/斤        19.80         65.76\n");
        SendDataString(" 冬瓜          2.5元/斤      10.98         65.76\n");
        SendDataString(" 尖椒          3.6元/斤      200.20        65.76\n");
        SendDataString(" 西红柿        8元/斤        8.22          65.76\n");
        SendDataString("------------------------------------------------------------------\n");

        SendDataString(" 共计货品: 15种\n");
        SendDataString(" 共计金额: 1650.00\n");
        SendDataString(" 抹零后金额: 1600.00\n");

        SendDataString("------------------------------------------------------------------\n");

        SendDataString(" 菜商: 小孙菜商\n");
        SendDataString(" 主营: 蔬菜批售兼营\n");
        SendDataString(" 菜商电话: 156 6215 0448\n");
     ...
     * @param
     */
    private void BluetoothPrintTest3() {
        SendDataByte(Command.ESC_Init);
